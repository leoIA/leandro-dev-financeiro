...
/**
 * @file    BethaAdapter.php
 * @package App\Nfse\Adapters
 * @since   2026.07.07
 * @author  Leandro DEV
 * @license Proprietary — MM Construtora
 * @brief   Adapter NFSe para prefeituras Betha Sistemas (ABRASF v2.03).
 *
 * Municípios atendidos:
 *   - Itabuna  (IBGE 2914802)
 *   - Ilhéus   (IBGE 2913606)
 *   - Jequié   (IBGE 2918001)
 *
 * Características do Betha:
 *   - XML ABRASF v2.03 (mesmo namespace do WebISS)
 *   - SOAPAction: "EmitirNfse" (forma curta, sem namespace)
 *   - Namespace do envelope: http://www.betha.com.br/nfse-web-services/nfse
 *   - Retorno: <GerarNfseResposta> com
 *     <ListaNfse><CompNfse><Nfse><Numero>, <CodigoVerificacao>, <Protocolo>
 *
...
class BethaAdapter implements NfseAdapterInterface
{
    use HttpTrait;

    /** @var string SOAPAction de emissão Betha (forma curta). */
    private const SOAP_ACTION_EMITIR = 'EmitirNfse';

    /** @var string SOAPAction de consulta Betha. */
    private const SOAP_ACTION_CONSULTAR = 'ConsultarNfse';

    /** @var string SOAPAction de cancelamento Betha. */
    private const SOAP_ACTION_CANCELAR = 'CancelarNfse';

    /** @var string Namespace SOAP do Betha. */
    private const NAMESPACE_NS = 'http://www.betha.com.br/nfse-web-services/nfse';
    ...
    {
        // 1) Constrói XML ABRASF via XmlBuilder (Betha = ABRASF_BETHA).
        try {
            $built = (new XmlBuilder())->build($nfse);
            $xml = $built['xml'];
        } catch (RuntimeException $e) {
            $this->logError('BethaAdapter::emitir(build)', $e->getMessage());
        ...
        try {
            ...
            $this->logError('BethaAdapter::emitir(sign)', $e->getMessage());
            return $this->fail($e->getMessage(), $xml, null);
        }

        // 3) Wrap em envelope SOAP (Betha usa nfse namespace próprio).
        ...
        if ($endpoint === null) {
            ...
            $this->logError('BethaAdapter::emitir(endpoint)', $msg);
        ...
        if ($resp['error'] !== null) {
            $this->logError('BethaAdapter::emitir(http)', $resp['error']);
        ...
        // 7) Extrai número, código de verificação e protocolo do retorno Betha.
        //    Estrutura esperada: <GerarNfseResposta><ListaNfse><CompNfse><Nfse>...
        ...
        if ($numero === null) {
            ...
            $this->logError('BethaAdapter::emitir(parse)', $msg . ' | body=' . substr($xmlRetorno, 0, 500));
    ...
    public function consultar(string $protocolo): array
    {
        ...
        $body = '<nfse:ConsultarNfseRequest xmlns:nfse="' . self::NAMESPACE_NS . '">'
        ...
        if ($resp['error'] !== null) {
            $this->logError('BethaAdapter::consultar', $resp['error']);
    ...
    public function cancelar(string $protocolo, string $motivo): array
    {
        ...
        if ($resp['error'] !== null) {
            $this->logError('BethaAdapter::cancelar', $resp['error']);
